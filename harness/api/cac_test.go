...
`)
	...
	item, err := c.ConfigAsCode().UpsertRawYaml(path, yamlBytes)
	require.NoError(t, err)
	require.NotNil(t, item)

	yamlItem, err := c.ConfigAsCode().FindYamlByPath(app.Id, path)
	require.NoError(t, err)
...
`)
	...
	item, err := c.ConfigAsCode().UpsertRawYaml(cac.YamlPath(path), yamlBytes)
	require.NoError(t, err)
	require.NotNil(t, item)

	yamlItem, err := c.ConfigAsCode().FindYamlByPath(app.Id, cac.YamlPath(path))
	require.NoError(t, err)
	require.NotNil(t, yamlItem)
	require.Equal(t, app.Id, yamlItem.ApplicationId)
...
`)
	...
	yamlItem, err := c.ConfigAsCode().FindYamlByPath("", path)
	require.NoError(t, err)
	require.NotNil(t, yamlItem)
	...
	err = c.ConfigAsCode().DeleteEntity(path)
	require.NoError(t, err)
}
...
`)
	...
	yamlBytes := []byte(rawYaml)
	rootPath, err := c.ConfigAsCode().GetTemplateLibraryRootPathName()
	require.NoError(t, err)
...
func TestGetTemplateLibraryRootPath(t *testing.T) {
	c := getClient()
	path, err := c.ConfigAsCode().GetTemplateLibraryRootPathName()
	require.NoError(t, err)
