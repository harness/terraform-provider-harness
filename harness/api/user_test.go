...
func createUserWithGroups(email string, groupIds []string) (*graphql.User, error) {
	...
	c := getClient()
	return c.Users().CreateUser(input)
}
...
func TestCreateUser(t *testing.T) {
	...
	c := getClient()
	err = c.Users().DeleteUser(user.Id)
	require.NoError(t, err)
}
...
func TestUpdateUser(t *testing.T) {
	...
	_, err = c.Users().UpdateUser(&graphql.UpdateUserInput{
		Id:   user.Id,
		Name: "updated_name",
	})
	require.NoError(t, err)

	updateUser, err := c.Users().GetUserByEmail(expectedEmail)
	require.NoError(t, err)
...
func TestGetUserByEmail(t *testing.T) {
	...
	user, err := c.Users().GetUserByEmail(expectedUser.Email)
	require.NoError(t, err)
	require.NotNil(t, user)
	require.Equal(t, expectedUser.Name, user.Name)
	require.Equal(t, expectedUser.Email, user.Email)
...
func TestGetUserById(t *testing.T) {
	...
	user, err := c.Users().GetUserById(expectedUser.Id)
	require.NoError(t, err)
	require.NotNil(t, user)
	require.Equal(t, expectedUser.Name, user.Name)
	require.Equal(t, expectedUser.Email, user.Email)
...
func TestListUsers(t *testing.T) {
	...
	for hasMore {
		users, pagination, err := client.Users().ListUsers(limit, offset)
