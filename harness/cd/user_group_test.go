...
func createUserGroup(name string) (*graphql.UserGroup, error) {
	...
	c := getClient()
	return c.UserClient.CreateUserGroup(&input)
}
...
func TestCreateUserGroup_NotificationSettings(t *testing.T) {
	...
	userGroup, err := c.UserClient.CreateUserGroup(&ug)
	...
	// c := getClient()
	err = c.UserClient.DeleteUserGroup(userGroup.Id)
	require.NoError(t, err)
}
...
func TestUpdateUserGroup(t *testing.T) {
	...
	updatedGroup, err := c.UserClient.UpdateUserGroup(userGroup)
	require.NoError(t, err)
...
func TestGetUserGroupById(t *testing.T) {
	...
	foundUG, err := c.UserClient.GetUserGroupById(userGroup.Id)
	require.NoError(t, err)
...
func TestGetUserGroupByName(t *testing.T) {
	...
	foundUG, err := c.UserClient.GetUserGroupByName(userGroup.Name)
	require.NoError(t, err)
	require.NotNil(t, foundUG)
	require.Equal(t, userGroup.Name, foundUG.Name)
...
func TestAddUserToUserGroup(t *testing.T) {
	...
	user, err := c.UserClient.GetUserByEmail("micahlmartin+testing@gmail.com")
	require.NoError(t, err)
	require.NotNil(t, user)

	ok, err := c.UserClient.AddUserToGroup(user.Id, userGroup.Id)
	require.NoError(t, err)
	require.True(t, ok)
	...
	ok, err = c.UserClient.IsUserInGroup(user.Id, userGroup.Id)
	require.NoError(t, err)
	require.True(t, ok)

	ok, err = c.UserClient.RemoveUserFromGroup(user.Id, userGroup.Id)
	require.NoError(t, err)
	require.True(t, ok)
...
func TestListUserGroups(t *testing.T) {
	...
	for hasMore {
		groups, pagination, err := client.UserClient.ListUserGroups(limit, offset)
...
func TestListUsersGroupMembership(t *testing.T) {
	...
	for hasMore {
		groups, pagination, err := c.UserClient.ListGroupMembershipByUserId(user.Id, limit, offset)
