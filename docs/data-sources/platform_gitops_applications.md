...
- `application` (Block List) definition of Application resource. (see [below for nested schema](#nestedblock--application))
...
- `metadata` (Block List) metadata that all persisted resources must have, which includes all objects users must create. (see [below for nested schema](#nestedblock--application--metadata))
