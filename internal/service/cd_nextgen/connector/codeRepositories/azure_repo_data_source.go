...
func DatasourceConnectorAzureRepo() *schema.Resource {
	resource := &schema.Resource{
		Description: "Datasource for looking up a Azure Repo connector.",
		ReadContext: resourceConnectorAzureRepoRead,

		Schema: map[string]*schema.Schema{
			"url": {
				Description: "URL of the azure repo repository or account.",
			...
			"api_authentication": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"token_ref": {
							Description: "Personal access token for interacting with the azure repo api." + secretRefText,
							Type:        schema.TypeString,
							Computed:    true,
						},
					},
