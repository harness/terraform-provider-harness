...
func TestAccResourceFMELargeSegmentEnvironmentAssociation_basic(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping FME large segment environment association acceptance test in short mode")
	}
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccResourceFMELargeSegmentEnvironmentAssociation(id, envName, lsName),
...
func testAccResourceFMELargeSegmentEnvironmentAssociation(id, envName, lsName string) string {
	return fmt.Sprintf(`
	...
	resource "harness_fme_large_segment" "test" {
		...
		name            = "%[3]s"
		description     = "acc large segment for env association"
