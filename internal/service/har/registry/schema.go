...
func resourceRegistrySchema(readOnly bool) map[string]*schema.Schema {
	mainSchema := map[string]*schema.Schema{
		...
		"package_type": {
			Description: "Type of package (DOCKER, HELM, HELM_HTTP, MAVEN, GO, CONDA, DEBIAN, RUBY etc.)",
			Type:        schema.TypeString,
			Required:    true,
			ValidateFunc: validation.StringInSlice([]string{
	...
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			"source": {
				...
				Description: "Upstream source",
				ValidateFunc: validation.StringInSlice([]string{
					"Dockerhub", "Custom", "AwsEcr", "MavenCentral", "PyPi", "NpmJs", "NugetOrg", "Crates", "GoProxy", "Anaconda", "HelmChartRepo", "RubyGems",
				}, false),
			},
			"url": {
				Type:         schema.TypeString,
