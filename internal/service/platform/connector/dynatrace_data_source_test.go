...
func TestAccDataSourceConnectorDynatrace(t *testing.T) {

	var (
		name         = fmt.Sprintf("%s_%s", t.Name(), utils.RandStringBytes(4))
		resourceName = "data.harness_platform_connector_dynatrace.test"
	)
...
func testAccDataSourceConnectorDynatrace(name string) string {
	...
	resource "harness_platform_secret_text" "test" {
		...
		data "harness_platform_connector_dynatrace" "test" {
			identifier = harness_platform_connector_dynatrace.test.identifier
		}
	`, name)
}
