...
func TestAccDataSourceConnectorDocker(t *testing.T) {

	var (
		name         = fmt.Sprintf("%s_%s", t.Name(), utils.RandStringBytes(4))
		resourceName = "data.harness_platform_connector_docker.test"
	)
...
func testAccDataSourceConnectorDocker(name string) string {
	return fmt.Sprintf(`
		...
		data "harness_platform_connector_docker" "test" {
			identifier = harness_platform_connector_docker.test.identifier
		}
	`, name)
}
