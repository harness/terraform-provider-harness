...
func TestAccDataSourceConnectorServiceNow_RefreshToken(t *testing.T) {
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceConnectorServiceNow_RefreshToken(name),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "id", name),
					resource.TestCheckResourceAttr(resourceName, "identifier", name),
					resource.TestCheckResourceAttr(resourceName, "name", name),
...
func testAccDataSourceConnectorServiceNow_RefreshToken(name string) string {
	return fmt.Sprintf(`
	resource "harness_platform_secret_text" "test" {
		identifier = "%[1]s"
		name = "%[1]s"
