...
func TestAccDataSourceConnectorAzureKeyVaultProjectLevel(t *testing.T) {
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceConnector_azureKeyVaultProjectLevel(name),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "id", name),
					resource.TestCheckResourceAttr(resourceName, "identifier", name),
					resource.TestCheckResourceAttr(resourceName, "name", name),
...
func TestAccDataSourceConnectorAzureKeyVaultOrgLevel(t *testing.T) {
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceConnector_azureKeyVaultOrgLevel(name),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "id", name),
					resource.TestCheckResourceAttr(resourceName, "identifier", name),
					resource.TestCheckResourceAttr(resourceName, "name", name),
...
func testAccDataSourceConnector_azureKeyVaultProjectLevel(name string) string {
	return fmt.Sprintf(`
	...
	data "harness_platform_connector_azure_key_vault" "test" {
		identifier = harness_platform_connector_azure_key_vault.test.id
		org_id = harness_platform_connector_azure_key_vault.test.org_id
		project_id = harness_platform_connector_azure_key_vault.test.project_id
		
	}
`, name)
}

func testAccDataSourceConnector_azureKeyVaultOrgLevel(name string) string {
	return fmt.Sprintf(`
	resource "harness_platform_organization" "test" {
		identifier = "%[1]s"
		name = "%[1]s"
