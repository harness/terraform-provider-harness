...
Architecture:
  ...
  Account A (Proxy Account)          Account B (Target Account)
  ┌─────────────────────┐            ┌─────────────────────────┐
  │  AutoStopping Proxy │◄── rule ───│  Target VM / ECS / RDS  │
  │  (access point)     │            │                         │
  └─────────────────────┘            └─────────────────────────┘
  connector: PROXY_CONNECTOR_ID      connector: TARGET_CONNECTOR_ID

Supported resource types:

  The proxy_cloud_connector_id field is supported on all rule types that
  have an http or tcp routing block:

    - harness_autostopping_rule_vm          (http + tcp)
    - harness_autostopping_rule_ecs         (http)
    - harness_autostopping_rule_rds         (tcp)
    - harness_autostopping_rule_scale_group (http)

  The cross-account proxy routing is handled in the shared getRoutingConfigurations
  (build) and setHttpConfig/setTcpConfig (read) functions in as_rule.go, so all
  rule types share the same code path. For this reason, only VM rules are tested
  here — HTTP and TCP cover both code paths.
