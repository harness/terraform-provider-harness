...
func TestAccDataSourceGitopsAgentOperatorYaml(t *testing.T) {
	agentId := fmt.Sprintf("%s_%s", t.Name(), utils.RandStringBytes(5))
	agentId = strings.ReplaceAll(agentId, "_", "")
	namespace := "ns-" + agentId
	accountId := os.Getenv("HARNESS_ACCOUNT_ID")
	resourceName := "data.harness_platform_gitops_agent_operator_yaml.test"
	resource.UnitTest(t, resource.TestCase{
		PreCheck:          func() { acctest.TestAccPreCheck(t) },
		ProviderFactories: acctest.ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceGitopsAgentOperatorYaml(agentId, accountId, agentId, namespace),
...
func testAccDataSourceGitopsAgentOperatorYaml(agentId string, accountId string, agentName string, namespace string) string {
	return fmt.Sprintf(`
