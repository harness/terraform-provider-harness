	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateIdFunc: acctest.ChaosTemplateImportStateIdFunc(resourceName),
			},
		},
	})
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "run_properties.0.timeout", "10s"),
				),
			},
			{
				Config: testAccResourceProbeTemplate_httpProbe_updated(name),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", name),
					resource.TestCheckResourceAttr(resourceName, "description", "Updated HTTP probe template"),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "3"),
					resource.TestCheckResourceAttr(resourceName, "run_properties.0.timeout", "15s"),
				),
			},
		},
	})
...
func testAccResourceProbeTemplate_httpProbe_updated(name string) string {
	return fmt.Sprintf(`
		resource "harness_platform_organization" "test" {
			identifier = "%[1]s"
			name       = "%[1]s"
