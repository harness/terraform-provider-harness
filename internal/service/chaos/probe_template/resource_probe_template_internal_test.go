...
// harness_chaos_probe_template resource. Deterministic; no live API / TF_ACC.

import "testing"

func TestParseProbeTemplateImportID(t *testing.T) {
	tests := []struct {
		name        string
		id          string
		wantOrg     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			org, project, hub, id, err := parseProbeTemplateImportID(tt.id)
			if tt.wantErr {
				if err == nil {
