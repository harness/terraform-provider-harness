...
func TestAccDataSourceChaosSecurityGovernanceRule(t *testing.T) {
	...
	rName := id
	dataSourceName := "data.harness_chaos_security_governance_rule_v3.test"
	resourceName := "harness_chaos_security_governance_rule_v3.test"
...
func testAccDataSourceChaosSecurityGovernanceRuleConfig(name, id, infraType string) string {
	...
	resource "harness_chaos_security_governance_rule_v3" "test" {
		org_id     = harness_platform_organization.test.id
		project_id = harness_platform_project.test.id
		name       = "%[1]s"
		...
		condition_ids = [harness_chaos_security_governance_condition_v3.test.id]
	...
	data "harness_chaos_security_governance_rule_v3" "test" {
		identity   = harness_chaos_security_governance_rule_v3.test.id
		org_id     = harness_platform_organization.test.id
		project_id = harness_platform_project.test.id
	}
	`, name, id, infraType)
...
func testAccDataSourceChaosSecurityGovernanceRuleConfigByName(name, id, infraType string) string {
	...
	data "harness_chaos_security_governance_rule_v3" "test" {
		...
		depends_on = [harness_chaos_security_governance_rule_v3.test]
	}
	`, name, id, infraType)
}
