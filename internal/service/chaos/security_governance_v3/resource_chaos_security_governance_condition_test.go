package security_governance_v3_test
...
func TestAccResourceChaosSecurityGovernanceCondition(t *testing.T) {
	...
	rName := id
	resourceName := "harness_chaos_security_governance_condition_v3.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "k8s_spec.0.application_spec.0.workloads.0.namespace_labels.app", "frontend"),
					resource.TestCheckResourceAttr(resourceName, "k8s_spec.0.application_spec.0.workloads.0.namespace_labels.tier", "web"),
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "k8s_spec.0.application_spec.0.workloads.0.namespace_labels.app", "backend"),
					resource.TestCheckResourceAttr(resourceName, "k8s_spec.0.application_spec.0.workloads.0.namespace_labels.tier", "api"),
				),
			},
		},
	})
...
func testAccResourceChaosSecurityGovernanceConditionConfig(name, id, infraType string) string {
	...
	resource "harness_chaos_security_governance_condition_v3" "test" {
		org_id     = harness_platform_organization.test.id
