...
// TestAccDataSourceFaultTemplate_byName is disabled due to API limitation
// The ListFaultTemplate endpoint returns 500 Internal Server Error when called
// immediately after resource creation. Use byIdentity lookup instead.
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", name),
					resource.TestCheckResourceAttr(resourceName, "identity", name),
					// Data source has simplified schema - only fault_name, not full spec
					resource.TestCheckResourceAttrSet(resourceName, "fault_name"),
					resource.TestCheckResourceAttrSet(resourceName, "hub_identity"),
				),
			},
		},
