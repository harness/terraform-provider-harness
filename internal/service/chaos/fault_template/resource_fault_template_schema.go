							...
							Schema: map[string]*schema.Schema{
								"fault_name": {
									...
									DiffSuppressFunc: func(k, old, new string, d *schema.ResourceData) bool {
										// Suppress diff if API returns a default value (known API limitation)
										// The API often returns "byoc-injector" as default regardless of what we send
										if old == "byoc-injector" && new != "" && old != new {
											// API returned default value, suppress the diff
											return true
										}
										return false
									},
