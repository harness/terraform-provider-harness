...
func TestAccDataSourceOrganization(t *testing.T) {
	...
	resourceName := "data.harness_organization.test"
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "identifier", fmt.Sprintf("%s_1", id)),
					resource.TestCheckResourceAttr(resourceName, "description", "test"),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "2"),
				),
			},
...
func testAccDataSourceOrganization(id string) string {
	return fmt.Sprintf(`
		...
		data "harness_organization" "test" {
			identifier = harness_organization.test.identifier
		}
	`, id)
}
...
func testAccDataSourceOrganization_SearchTerm(id string) string {
	return fmt.Sprintf(`
		...
		data "harness_organization" "test" {
			search_term = harness_organization.test.name
		}
	`, id)
}
...
func testAccDataSourceOrganization_MultipleResults(id string, selectFirst bool) string {
	return fmt.Sprintf(`
		resource "harness_organization" "test_1" {
			identifier = "%[1]s_1"
			name = "%[1]s_1 testing"
			description = "test"
			tags = ["foo:bar", "baz:qux"]
		...
		resource "harness_organization" "test_2" {
			identifier = "%[1]s_2"
			name = "%[1]s_2 testing"
			description = "test"
			tags = ["foo:bar", "baz:qux"]
